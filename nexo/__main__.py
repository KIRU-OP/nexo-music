...
import config
from nexo import LOGGER, app, userbot
from nexo.core.call import JARVIS
from nexo.misc import sudo
from nexo.plugins import ALL_MODULES
from nexo.utils.database import get_banned_users, get_gbanned
from nexo.utils.lyrics import close_lyrics_http_client
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("nexo.plugins" + all_module)

    LOGGER("nexo.plugins").info("Modules loaded.")
    LOGGER("nexo").info(
    ...
    try:
        ...
        pass
    try:
        song_plugin = importlib.import_module("nexo.plugins.tools.song")
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("nexo").info("Stopping music bot...")
